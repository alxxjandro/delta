...
export default function NewCoverLetter() {
  /**
   * State to control the loading and error states
   */
  ...
  const form = useForm<ICoverLetter>({
    ...
    validate: {
      title: (value) => value.trim().length <= 0 && 'The title cannot be empty!',
      company: (value) => value.trim().length <= 0 && 'The company cannot be empty!',
      position: (value) => value.trim().length <= 0 && 'The position cannot be empty!',
      keypoints: (value) => value.trim().length <= 0 && 'Add at least one keypoint!',
      tone: (value) => (value ? null : 'Tone is required'),
    },
  })
  ...
  const handleSubmit = form.onSubmit(async (data: ICoverLetter) => {
    try {
      setLoading(true)
      const response = await CLS.generate(data)
    ...
    <>
      ...
      <Stack bg="gray.0" mih="100dvh" gap={0}>
        ...
        <Container size="lg" h="100%">
          <Stack gap="xl" pt="10dvh" justify="start">
            <Stack gap={4}>
